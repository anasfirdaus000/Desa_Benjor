...
const KelolaBerita = () => {
  ...
  const handleOpenModal = (item: any = null) => {
    ...
    if (item) {
      ...
      setEditingId(null);
      setFormData({ title: '', category: '', summary: '', content: '', align: 'center' });
      setAdditionalPhotos([]);
      clearImage();
    }
    setIsModalOpen(true);
                ...
                <tr key={item.id} className="border-b dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-gray-855/30 transition-colors">
                  <td className="p-4">
                    <img 
                      src={item.image} 
                      alt={item.title} 
                      className={`w-20 h-14 rounded object-cover border dark:border-gray-850 bg-gray-105 ${
                  ...
                  <td className="p-4">
                    <div className="font-bold text-gray-850 dark:text-white line-clamp-1">{item.title}</div>
                  ...
                  <td className="p-4 text-sm text-gray-600 dark:text-gray-450">
                  ...
                  <td className="p-4">
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => handleDelete(item.id)} className="p-1.5 text-red-600 hover:bg-red-50 dark:hover:bg-red-955/40 rounded transition-colors cursor-pointer"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <div className="fixed inset-0 bg-black/60 dark:bg-black/75 flex items-center justify-center z-50 p-4 backdrop-blur-xs">
          <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-3xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border dark:border-gray-800">
            <div className="p-4 border-b dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-850">
              <h3 className="font-bold text-base text-gray-900 dark:text-white">{editingId ? 'Edit Berita' : 'Tulis Berita'}</h3>
            ...
            <div className="p-5 overflow-y-auto text-gray-800 dark:text-gray-250 flex-grow">
              {activeTab === 'info' ? (
                <form id="berita-form" onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-303 mb-1">Judul Berita</label>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-303 mb-1">Kategori</label>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-303 mb-1">Focal Point Gambar Utama (Fokus Foto)</label>
                    <select value={formData.align} onChange={e => setFormData({...formData, align: e.target.value})} className="w-full p-2.5 border dark:border-gray-750 bg-white dark:bg-gray-800 dark:text-white rounded-lg focus:ring-2 focus:ring-[#2D5A27] outline-none">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-303 mb-1">Ringkasan Singkat</label>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-303 mb-1">Isi Berita Lengkap</label>
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 dark:text-gray-303 mb-1">Gambar Utama (Sampul)</label>
                    ...
                    {previewUrl && (
                      <div className="relative mt-3 h-44 rounded-lg overflow-hidden border dark:border-gray-800 shadow-inner bg-gray-900 flex items-center justify-center">
                        <img 
                          src={previewUrl} 
                          alt="Preview" 
                        ...
                        />
                        {isUploading && (
                          <div className="absolute inset-0 bg-black/40 flex items-center justify-center text-xs text-white font-bold animate-pulse">
                ...
                <div className="space-y-4">
                  ...
                  {additionalPhotos.length === 0 ? (
                    <p className="text-xs text-gray-505 dark:text-gray-450 italic py-4 text-center">Belum ada foto tambahan yang diunggah.</p>
                  ) : (
                        ...
                        <div key={index} className="relative group rounded-lg overflow-hidden border dark:border-gray-800 bg-gray-105 h-28 shadow-sm">
                          ...
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                            <button 
                              ...
                              className="bg-red-600 hover:bg-red-700 text-white p-1.5 rounded-full transition-colors cursor-pointer shadow shadow-red-950/20"
                            >
                              <Trash2 size={16} />
                            </button>
                          </div>
            ...
            <div className="p-4 border-t dark:border-gray-800 bg-gray-50 dark:bg-gray-850 flex justify-end gap-2">
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-250 dark:hover:bg-gray-800 rounded-lg font-medium transition-colors cursor-pointer text-xs">Batal</button>
              {activeTab === 'info' ? (
                <button type="submit" form="berita-form" disabled={isUploading} className="px-5 py-2 bg-[#2D5A27] hover:bg-green-700 text-white rounded-lg font-bold transition-colors cursor-pointer text-xs disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
                  {isUploading ? 'Mengunggah...' : 'Simpan Publikasi'}
                </button>
              ) : (
                <button onClick={handleSubmit} disabled={isUploading || isUploadingPhoto} className="px-5 py-2 bg-[#2D5A27] hover:bg-green-700 text-white rounded-lg font-bold transition-colors cursor-pointer text-xs shadow-sm">
                  Simpan Publikasi
                </button>
              )}
            </div>

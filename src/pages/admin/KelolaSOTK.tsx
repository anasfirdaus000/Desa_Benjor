...
const KelolaSOTK = () => {
  ...
  const handleOpenModal = (item: any = null) => {
    if (item) {
      ...
      setFormData({ 
        name: item.name, position: item.position, level: item.level
      });
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 text-gray-800 dark:text-gray-250 transition-colors duration-300">
            ...
            <tbody>
              {sotk.map(member => (
                <tr key={member.id} className="border-b dark:border-gray-800 hover:bg-gray-50/50 dark:hover:bg-gray-850/30 transition-colors">
                  <td className="p-4">
                    <img src={member.image} alt={member.name} className="w-12 h-12 rounded-full object-cover border dark:border-gray-800 bg-gray-100" />
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => handleDelete(member.id)} className="p-1.5 text-red-605 hover:bg-red-50 dark:hover:bg-red-950/40 rounded transition-colors cursor-pointer"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/55 dark:bg-black/75 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-900 rounded-xl max-w-xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[90vh] border dark:border-gray-800">
            <div className="p-4 border-b dark:border-gray-800 flex justify-between items-center bg-gray-50 dark:bg-gray-850">
              <h3 className="font-bold text-lg text-gray-900 dark:text-white">{editingId ? 'Edit Anggota SOTK' : 'Tambah Perangkat Desa'}</h3>
            ...
            <div className="p-4 border-t dark:border-gray-800 bg-gray-55 dark:bg-gray-850 flex justify-end gap-2">
              ...
              <button type="submit" form="sotk-form" disabled={isUploading} className="px-4 py-2 bg-[#2D5A27] hover:bg-green-700 text-white rounded-lg font-medium transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
